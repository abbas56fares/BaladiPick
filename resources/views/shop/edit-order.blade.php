                ...
                <form action="{{ route('shop.orders.update', $order->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('vehicle_type') is-invalid @enderror" 
                            ...
                            <option value="bike" {{ old('vehicle_type', $order->vehicle_type) == 'bike' ? 'selected' : '' }}>Bike</option>
                            <option value="car" {{ old('vehicle_type', $order->vehicle_type) == 'car' ? 'selected' : '' }}>Car</option>
                        </select>
                        @error('vehicle_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const startLat = currentLat ? parseFloat(currentLat) : shopLat;
    const startLng = currentLng ? parseFloat(currentLng) : shopLng;
